...
if getgenv().customdrawingLoaded then return warn("Wave Init Script Already Executed") end
warn("Wave Init Script Executed")
...
getgenv().setfpscap = newcclosure(function(bruh)
	...
	if SetTo == 0 then
		SetTo = SetTo > 0 and 1.0 / SetTo or 1.0 / 10000.0
	end
...
do
	...
	do
		...
		function square:_updateScale()
			...
			self._frame.Position =
				_udim2fromoffset(props.Position.X + props.Thickness, props.Position.Y + props.Thickness)
	...
	do
		...
		function quad:__newindex(k, v)
			if self.__OBJECT_EXISTS == true then
				...
				if k == "Color" then
					...
					frame._fill.BackgroundColor3 = v
				elseif k == "Filled" then
					frame._fill.Visible = v
				elseif k == "PointA" then
					self:_updateVertices({
						{ frame._line1, props.PointA, props.PointB },
		...
		function quad:_calculateFill()
			local props, frame = self._properties, self._frame
			local a, b, c, d = props.PointA, props.PointB, props.PointC, props.PointD
			local center = (a + b + c + d) / 4
			local sizeX = (a - b).Magnitude
			local sizeY = (a - d).Magnitude
			frame._fill.Position = _udim2fromoffset(center.X, center.Y)
			frame._fill.Size = _udim2fromoffset(sizeX, sizeY)
		end
...
genv.getrenderproperty = newcclosure(function(x, y)
	_assert(
		_isrenderobj(x),
		_stringformat("invalid argument #1 to 'getrenderproperty' (Drawing expected, got %s)", _typeof(x))
	)
	return x[y]
end)
...
genv.setrenderproperty = newcclosure(function(x, y, z)
	_assert(
		_isrenderobj(x),
		_stringformat("invalid argument #1 to 'setrenderproperty' (Drawing expected, got %s)", _typeof(x))
	)
	x[y] = z
end)
